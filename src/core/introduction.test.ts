...
test("Runtime starts a battle only from a trusted request with present allies", async () => {
  const initial = state();
  initial.characters = { ...initial.characters, mash: { id: "mash", locationId: "station", mood: "alert" } };
  const runtime = new GameRuntime(initial, {});
  const result = await runtime.startBattle({
  ...
  await assert.rejects(runtime.startBattle({
  ...
  await assert.rejects(absentRuntime.startBattle({
