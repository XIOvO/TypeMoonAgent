...
这是一个由 Agent 驱动、可组合运行的 Web 游戏运行时（Agent-driven, composable Web Game Runtime）。

- **Agent**：理解玩家意图、扮演角色、基于已知信息作出判断，并提出行动。
- **Game Runtime**：保存客观事实、验证规则、推进时间与世界，并裁决 Agent 提议的行动。
- **运行与组合层**：管理可选游戏能力的依赖、启动、停止与后台任务；它不拥有世界事实，也不替代 Runtime 裁决。
...
冷路径的输入只能来自已确认的 `GameEvent` 或 Runtime 创建的世界推进任务。每项异步工作必须以持久化 `Job` / Outbox 记录意图、幂等键、状态与重试次数；不能只依赖进程内回调，避免重启或模型不可用时漏掉工作。

### 运行与组合层：能力，而非世界实体

论文带来的新增层解决“系统现在能做什么、何时运行、如何安全退出”，而不重新定义角色或世界。可选能力（如记忆整合、战斗、章节导演、图像生成）应声明：

```text
requires：所需端口或能力（Event Repository、模型、规则端口等）
provides：对系统提供的能力（如 memory_consolidation）
start：注册 Worker、处理器或受控工具
stop：撤销注册、停止接收新任务、清理临时资源
```

`Runtime`、事件日志、世界状态、CIF 仓储是基础设施，不应被可选模块动态替换。第一阶段采用显式注册表和组合根装配即可；不引入通用动态代码加载或插件市场。

### 按需世界推进

角色持续存在于数据库，不意味着每个 NPC 都持续调用模型。世界时间推进、玩家回归或章节压力变化时，Runtime 先以确定性规则筛出少量受影响角色，再创建可重试的 `SimulationJob`；角色 Agent 仅为该任务提出意图，最终变化仍由 Runtime 写为 `GameEvent`。
