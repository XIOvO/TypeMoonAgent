...
| CIF | 身份、证据、认识、解释模型、即时状态；最小 Context Builder | 长期记忆整合、信念更新策略仍待实现 |
| 持久化 | SQLite 世界快照、客观事件、CIF、幂等记录与事务 | migration 框架和重启后完整幂等恢复待补 |
...
4. **章节世界推进**：为冬木先建立少量 `CanonBeat` 与 `WorldBranch`，让偏离原作后危机仍按因果推进。
